...
import { redeemQrImageUrl } from '../lib/redeemQr';
import { 
  User as UserIcon, 
  Globe, 
  FileText, 
...
export const SettingsView: React.FC<SettingsViewProps> = ({
  ...
  const [lastName, setLastName] = useState(currentUser?.last_name && currentUser.last_name !== '-' ? currentUser.last_name : '');
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      setLastName(currentUser.last_name && currentUser.last_name !== '-' ? currentUser.last_name : '');
  ...
  const handleAvatarFile = async (file: File | undefined) => {
    ...
    if (!file.type.startsWith('image/')) {
      addToast('error', 'ไฟล์ไม่ถูกต้อง', 'กรุณาเลือกรูป JPG PNG หรือ WEBP');
      return;
    }
    if (file.size > 2 * 1024 * 1024) {
      addToast('error', 'ไฟล์ใหญ่เกินไป', 'ใช้รูปขนาดไม่เกิน 2 MB');
      return;
    }
    ...
    reader.onerror = () => {
      setUploadingAvatar(false);
      addToast('error', 'อ่านไฟล์ไม่สำเร็จ', 'ลองเลือกรูปใหม่');
    };
              ...
              <form onSubmit={handleSaveProfile} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-100">
                    ...
                    <div className="flex-1 space-y-1.5 min-w-0">
                      <span className="text-[11px] text-slate-500 block">{language === 'th' ? 'อัปโหลดรูปจากเครื่อง หรือเลือกแบบสำเร็จรูป' : 'Upload a photo or pick a preset'}</span>
                      <label className="inline-flex items-center gap-1.5 text-[11px] font-bold text-emerald-800 bg-white border border-emerald-200 px-2.5 py-1.5 rounded-lg hover:bg-emerald-50 cursor-pointer">
                        <Upload className="w-3.5 h-3.5" />
                        {language === 'th' ? 'อัปโหลดรูปโปรไฟล์' : 'Upload photo'}
                        <input
                          type="file"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  <div className="space-y-1">
                    <label className="block font-semibold text-slate-700">{language === 'th' ? 'ชื่อ (First name):' : 'First name:'}</label>
                    <input 
                      type="text"
                      required
                      value={firstName}
                  ...
                  <div className="space-y-1">
                    <label className="block font-semibold text-slate-700">{language === 'th' ? 'นามสกุล (Last name):' : 'Last name:'}</label>
                    <input 
                      type="text"
                      required
                      value={lastName}
          ...
          >
            ...
            <div className="space-y-3">
              ...
              <div className="p-3 bg-slate-50 rounded-2xl border border-slate-200 flex flex-col items-center justify-center space-y-1.5">
                <div className="w-36 h-36 bg-white p-2 rounded-xl border border-slate-300 flex items-center justify-center shadow-2xs">
                  <img
                    src={redeemQrImageUrl(selectedRedemption.pickup_code, 220)}
                    alt={selectedRedemption.pickup_code}
                    className="w-full h-full object-contain"
                  />
                </div>
                <span className="text-[10px] text-slate-400">
                  {selectedRedemption.redeem_status === 'สำเร็จ'
                    ? (language === 'th' ? 'รับของรางวัลแล้ว' : 'Already claimed')
                    : (language === 'th' ? 'ให้แอดมินสแกน QR นี้ที่จุดรับของ' : 'Show this QR to admin at pickup')}
                </span>
              </div>
